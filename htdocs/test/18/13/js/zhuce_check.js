...
//密码
         if (myform.zc_password.value == "" || myform.zc_password.value.length<3){		  		
		alert("密码不能少于3位！");
...
//确认密码
         ...
         if (myform.mmqr.value == "" || myform.mmqr.value.length<3){
		alert("请输入确认密码");
		myform.remima.focus();
		return (false);	 
         }
...
function user_check(form){
	if (myform.zc_username.value!="" && myform.zc_username.value.length>=3){
		 form.action="";
		 form.submit();
	 }else{
		  alert("用户名不能为空且不能于少3个字符");
		}
}
